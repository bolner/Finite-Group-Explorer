...
class AssocHeuristics {
    private:
        ...
        inline int FindPossibleValue() {
            ...
            uint8_t left, y_i, right, i_x;

            for(uint8_t i = 0; i < this->order; i++) {
                /*
                    Right associative checks (x*y)*i = x*(y*i)
                */
                ...
                y_i = this->Mult(this->y, i);
                if (y_i == 0) {
                    goto left_checks;   // No information
                }

                right = this->Mult(this->x, y_i - 1);
                if (right == 0) {
                    goto left_checks;   // No information
                }
                ...
                /*
                    Left associative checks i*(x*y) = (i*x)*y
                */
                left_checks:
                ...
                i_x = this->Mult(i, this->x);
                if (i_x == 0) {
                    continue;   // No information
                }

                right = this->Mult(i_x - 1, this->y);
                if (right == 0) {
                    continue;   // No information
                }
